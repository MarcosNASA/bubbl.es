...
const Tuto = styled.div`
  ...
  font-size: 15px;
  line-height: 1.5;
  box-shadow: 9px 6px 6px rgba(0, 0, 0, 0.4);
  position: relative;
  ...
  :before {
    ...
    border-bottom: 15px solid ${colors.dark[100]};
    border-radius: 2px;
  }
`
...
const Theory = () => {
  ...
  return total;
        ...
        <Section>
          ...
          <Paragraph>
            We can take advantage of this information in order to predict and visualize the scopes just from looking at
            our code!
          </Paragraph>
        </Section>
        <Section>
          <Header>
            <SectionTitle ref={howRef}>How do we visualize the scope chain?</SectionTitle>
          </Header>
          <Paragraph>
          ...
          <Paragraph>
            In order to predict scopes, thus, we will have to think like the JS engine does! Every time we find a block
            in our code, we will assign a color to it. Every time we find a variable, we will also assign a color to it.
            But in order to do that, we need to dive into some concepts!
          </Paragraph>
          <Section>
            <Header>
...
addOdds(...array); // "array" is a source reference`}
          ...
          <Section>
            ...
            <Paragraph>
              However, there are other variables in the target roles that might join our variable declaration group, as
              in function parameters or loop declarations.
            </Paragraph>
            <CodeBox>
              <Code>
                {`function add(a, b) {
  ...
  return a + b;
          ...
          <Section>
            ...
            <Paragraph>
              ...
              variable declaration. The <var>var</var> variable declaration declares a{' '}
              <var>function scoped variable</var>. Function scoped variables set the bounds for their availability to
              the nearest enclosing function and, as we discussed previously, the nested blocks.
            </Paragraph>
            <Paragraph>
              ...
              set the bounds for their availability to the nearest enclosing block (whether it is a function, a pair of
          ...
          <Section>
            ...
            <Paragraph>
              ...
              assign a color to our variables can be summarized in two main ideas:
              <br />— Variable declarations will always have the color of the block where they are being declared,
              attending to the differences that using block or function scoped variables might carry. We will always
              consider function parameters as declarations inside the function scope.
              <br />— Whenever we find a variable in a source position or a variable in a target position which is not a
              variable declaration, we will perform a lookup for its declaration, starting in the current block all the
              way up to the outermost. We will stop as soon as we find it and will assign to it the color of the block
              where it was declared.
            </Paragraph>
            <Paragraph>
              With our current knowledge, we should be ready to test our new gained scope analysis skills!
              <br />
              Try revisiting our <var>addOdds</var> function and assign a colored bubble to each scope and variable we
              can find!
            </Paragraph>
            <details>
...
function addOdds(...numbers) { // 1st block start
  ...
  return total;
        ...
        <Section>
          <Header>
            <SectionTitle ref={aboutRef}>About this project</SectionTitle>
          </Header>
          <Paragraph>
          ...
          {({ dismiss, getSheepChildProps, goNextSheep }) => (
            <Tuto {...getSheepChildProps()}>
              ...
              <ButtonContainer topMargin={15}>
                ...
                <Button onClick={goNextSheep} isSmall>
                  Next
                </Button>
              </ButtonContainer>
            </Tuto>
        ...
        <Sheep spotRef={aboutRef}>
          {({ dismiss, activeSheep, getSheepChildProps, goPreviousSheep }) => (
            <Tuto {...getSheepChildProps()}>
              ...
              specific execution context or region of the code.
              <ButtonContainer topMargin={15}>
                <Button onClick={goPreviousSheep} isSmall>
                  Previous
                </Button>
                <Button onClick={dismiss} isSmall>
                  Finish
